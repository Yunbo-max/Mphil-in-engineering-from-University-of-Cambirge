...
sup_stock = pd.read_excel("Assignment_DA_2_a_data1.xlsx", sheet_name="Supplier stock", index_col=0)
raw_material_cost = pd.read_excel("Assignment_DA_2_a_data1.xlsx", sheet_name="Raw material costs", index_col=0)
raw_material_shipping = pd.read_excel("Assignment_DA_2_a_data1.xlsx", sheet_name="Raw material shipping", index_col=0)
production_req = pd.read_excel("Assignment_DA_2_a_data1.xlsx", sheet_name="Product requirements", index_col=0)
production_capacity = pd.read_excel("Assignment_DA_2_a_data1.xlsx", sheet_name="Production capacity", index_col=0)
customer_demand = pd.read_excel("Assignment_DA_2_a_data1.xlsx", sheet_name="Customer demand", index_col=0)
production_cost = pd.read_excel("Assignment_DA_2_a_data1.xlsx", sheet_name="Production cost", index_col=0)
shipping_costs = pd.read_excel("Assignment_DA_2_a_data1.xlsx", sheet_name="Shipping costs", index_col=0)
production_hour = pd.read_excel("Assignment_DA_2_a_data1.xlsx", sheet_name="Production hour", index_col=0)
...
production_hour = production_hour.fillna(0)
...
for factory in factories:
    for customer in customers:
        for product in products:
            delivery[(factory, customer, product)] = solver.IntVar(0, solver.infinity(),
...
a = [210000,120000,190000,200000]
for factory in factories:
    c = solver.Constraint(0, a[factories.index(factory)])
    for product in products:
        c.SetCoefficient(production_volume[(factory, product)], int(production_hour.loc[product][factory]))
...
for factory in factories:
    for customer in customers:
        for product in products:
            cost.SetCoefficient(delivery[(factory, customer, product)], 30*int(shipping_costs.loc[factory][customer])/3)
